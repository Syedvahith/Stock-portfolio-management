...
os.environ["API_KEY"] ="Enter your Api token"
...
def login():
    ...
    if request.method == "POST":
        ...
        # Check if the submitted credentials match the hardcoded values
        if submitted_username == hardcoded_username and submitted_password == hardcoded_password:
            # Set the user_id in the session (this is just for testing)
            session["user_id"] = 1  # You can set it to any value that represents a logged-in user
